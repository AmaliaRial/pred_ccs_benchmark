    ...
    # IF OK -> return properties
        ...
        try:
            ...
            if(error_code == 429):
                print("DENTRO")
                time.sleep(30)
                Glycerolipids, Glycerophospholipids, Sphingolipids, Steroids,
                Nucleoside_nucleotide, Fatty_acids, Acyl_carnitines, Monosaccharides,
                Organic_acids, Carboxylic_acids, Benzene=post_query_classyfire(smiles)
            else:
                ...
                data = {"query_input": smiles, "label": label, "query_type":
                "STRUCTURE"}
                headers = {"Content-Type": "application/json", "Accept":
                "application/json"}
                #print(data,headers)
                try:
                    r = requests.post(url = urlQueriesClassyfire, data = json.dumps(data),
                    headers = headers)
                    print(r.text)
                except Exception as e:
                    traceback.print_exc()
        ...
        return Glycerolipids, Glycerophospholipids, Sphingolipids, Steroids, Nucleoside_nucleotide, Fatty_acids, Acyl_carnitines, Monosaccharides, Carboxylic_acids, Benzene, Nucleoside_nucleotide, Fatty_acids, Acyl_carnitines, Monosaccharides, Carboxylic_acids, Benzene
